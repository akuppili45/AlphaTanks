...
/**
 * Manages the key pressed and released and what it does.
 * 
...
public class KeyInput extends KeyAdapter {
	...
	public void keyPressed(KeyEvent e) {
		...
		for (int i = 0; i < handler.object.size(); i++) {
			...
			if (tempObject.getID() == ObjectId.PlayerTank) { // Checks if the id
																// is a player
				PlayerTank p = (PlayerTank) tempObject;
				if (key == KeyEvent.VK_D) {
					p.setVelX(velocity);
				}
				// System.out.println("Tank Pos: " + tempObject.getX() + "," +
				// tempObject.getY());
			...
			if (tempObject.getID() == ObjectId.Gun) {
				...
				if (key == KeyEvent.VK_SPACE) {
					if (gun.isPlayerTank()) {
						// handler.addObject(new Bullet(((float) ((gun.x) +
						// gun.getRadius() *
						// Math.cos(Math.toRadians(gun.getAngle())))), (float)
						// (gun.y - gun.getRadius() *
						// Math.sin(Math.toRadians(gun.getAngle()))),
						// ObjectId.Bullet, (float) (20 *
						// Math.cos(Math.toRadians(gun.getAngle()))), gun,
						// handler));
